...
// In-memory store keyed by token (works for single instance free tier)
...
// Temp store for PKCE state (keyed by state param)
...
app.get('/callback', async (req, res) => {
  ...
  try {
    ...
    function postEarnings(tweet) {
      ...
      return { likes, rt, rep, bm, imp: imp || Math.round(reach), score: Math.round(score), revenue: parseFloat(((reach * premPct / 1000) * cpm * 0.525).toFixed(4)) };
    }
    ...
    const daysSinceLastPost = lastPostDate ? Math.floor((Date.now() - lastPostDate) / (1000 * 60 * 60 * 24)) : 999;
    ...
    const activeRecently = activeInLast30;
    const isEligible = isVerified && hasFollowers && activeRecently;
    ...
    if (!activeRecently) eligibilityReasons.push('No posts in the last 30 days — account must be active');
    if (totalImp < 5000000) eligibilityReasons.push('Est. ' + (totalImp / 1000000).toFixed(1) + 'M impressions in 90 days (5M needed)');
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      payoutHistory.push({ month: d.toLocaleString('en-US', { month: 'long' }), year: d.getFullYear(), amount: parseFloat((weeklyBase * 4.33 * v).toFixed(2)) });
    }
    ...
    const profile = {
      ...
      followers, following: user.public_metrics.following_count,
      tweet_count: user.public_metrics.tweet_count,
      avg_likes: avgLikes, avg_rt: avgRt, avg_replies: avgRep, avg_bookmarks: avgBm,
      posts_per_week: postsPerWeek, premium_pct: 10,
      biweekly_posts: posts, biweekly_count: posts.length,
